...
import { resolveCurrentBoardColumnSince, resolveIssueBoardColumn } from '../boardFlow.js';
import { clearIssueTimelineCache, fetchIssueTimeline } from '../issueTimeline.js';
...
export function clearFollowupSessionCaches() {
    clearIssueTimelineCache();
}

export { fetchIssueTimeline };
...
export function buildFollowupItems({
    ...
    const engineerMap = buildEngineerMap(issues, boardFlow, spHours, sprintHoursLeft);
    const pendingPrKeySet = new Set(pendingPrKeys);

    const items = issues.map(issue => {
        const issueKey = issue.key;
        const boardColumn = resolveIssueBoardColumn(issue, boardFlow);
        const timeline = buildIssueTimeline(issue, timelinesByKey[issueKey], boardFlow);
        ...
        const boardColumnAgeHours = timeline.currentBoardColumnSince
            ? workingHoursElapsed(timeline.currentBoardColumnSince, hoursPerDay, nowDate)
            : 0;
        ...
        const isAgeTrackedColumn = Boolean(boardColumn && !boardColumn.isDone && !boardColumn.isTodoLike);
        const signals = [];

        if (trackingMeta.state === 'blocked' || boardColumn?.isBlocked) signals.push('blocked');
        if (reminderTs && reminderTs <= now + (4 * 60 * 60 * 1000)) signals.push('reminder-due');
        if (prSignalsEnabled && isAgeTrackedColumn && !boardColumn?.isBlocked && !pr && !prPending && boardColumnAgeHours >= 4) signals.push('needs-pr');
        if (prSignalsEnabled && boardColumn?.isReviewLike && pr && (pr.state === 'open' || pr.draft) && prIdleHours >= 12) signals.push('review-waiting');
        if (isAgeTrackedColumn && !boardColumn?.isBlocked && statusIdleHours >= freezeThreshold && (!prSignalsEnabled || !pr || prIdleHours >= freezeThreshold)) {
            signals.push('frozen');
        }
        if (engineer?.capacityPct >= 100 && boardColumn?.isDone !== true) signals.push('capacity-risk');
...
function buildEngineerMap(issues, boardFlow, spHours, sprintHoursLeft) {
    const map = {};

    issues.forEach(issue => {
        const boardColumn = resolveIssueBoardColumn(issue, boardFlow);
        if (boardColumn?.isDone) return;
...
function buildIssueTimeline(issue, rawTimeline = {}, boardFlow) {
    ...
    const currentStatusSince = [...statusChanges].reverse().find(change =>
        (change.toId && currentStatusId && String(change.toId) === String(currentStatusId))
        || change.to === currentStatusName
    )?.created || issue.fields?.updated || null;

    return {
        statusChanges,
        currentStatusSince,
        currentBoardColumnSince: resolveCurrentBoardColumnSince(issue, statusChanges, boardFlow),
...
function selectPrimarySignal(signals = []) {
    ...
    return signals.slice().sort((left, right) => FOLLOWUP_SIGNAL_META[left].priority - FOLLOWUP_SIGNAL_META[right].priority)[0];
}

function compareFollowupItems(left, right) {
    if (left.sortRank.pinned !== right.sortRank.pinned) return left.sortRank.pinned - right.sortRank.pinned;
    if (left.sortRank.priority !== right.sortRank.priority) return left.sortRank.priority - right.sortRank.priority;
    if (left.sortRank.age !== right.sortRank.age) return right.sortRank.age - left.sortRank.age;
...
function describeSignal(signal, context) {
    ...
    const columnLabel = boardColumn?.name || 'current column';

    if (signal === 'blocked') {
        if (trackingMeta.state === 'blocked' && boardColumn?.isBlocked) {
            return `Blocked in board column "${columnLabel}" and flagged for follow-up`;
        }
        if (trackingMeta.state === 'blocked') return 'Flagged as blocked for follow-up';
        return `Current board column is "${columnLabel}"`;
    }
    ...
    if (signal === 'needs-pr') {
        return `No PR after ${formatHours(boardColumnAgeHours || 0)} in column ${columnLabel}`;
    }

    if (signal === 'review-waiting') {
        const reviewState = pr?.lastReviewState ? ` · last review ${humanizeReviewState(pr.lastReviewState)}` : '';
        return `PR idle for ${formatHours(prIdleHours || 0)} while ticket stays in ${columnLabel}${reviewState}`;
    }

    if (signal === 'frozen') {
        if (!pr) return `No Jira status change for ${formatHours(statusIdleHours || 0)} in ${columnLabel}`;
        return `No Jira status change or PR activity for ${formatHours(Math.max(statusIdleHours || 0, effectivePrIdleHours || 0))} in ${columnLabel}`;
    }
    ...
    if (signal === 'tracked-only') {
        ...
        if (timeline.currentBoardColumnSince) return `Tracked ticket in ${columnLabel}`;
...
function getSignalAge(signal, context) {
    const { now, reminderTs, boardColumnAgeHours, statusIdleHours, prIdleHours, trackingMeta } = context;
    const effectivePrIdleHours = Number.isFinite(prIdleHours) ? prIdleHours : statusIdleHours;

    if (signal === 'reminder-due' && reminderTs) {
        return reminderTs <= now ? (now - reminderTs) / (1000 * 60 * 60) : 0;
    }
    if (signal === 'needs-pr') return boardColumnAgeHours || 0;
